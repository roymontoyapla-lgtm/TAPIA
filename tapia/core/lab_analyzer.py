...
Se te proporcionaran una o varias imagenes que forman PARTE DEL MISMO analisis clinico
(por ejemplo, las distintas paginas de un mismo informe de laboratorio).
Combina la informacion de TODAS las imagenes en un unico resultado consolidado.
...
def extract_lab_values(
    ...
    `images` es una lista de tuplas (image_bytes, mime_type), una por pagina.
    Para mantener compatibilidad, tambien acepta bytes sueltos (una sola pagina).
    Devuelve un dict con los valores extraidos, combinando todas las paginas.
    """
    # Compatibilidad: si se pasa bytes sueltos (una sola imagen), lo envolvemos
    if isinstance(images, (bytes, bytearray)):
        images = [(images, mime_type)]

    if not images:
        return _fallback("No se proporciono ninguna imagen")
    ...
    try:
        ...
        # Construir el contenido: una imagen por pagina, en orden
        ...
        for img_bytes, img_mime in images:
            content.append({
                ...
                "source": {
                    "type": "base64",
                    "media_type": img_mime,
                    "data": _encode_image(img_bytes, img_mime),
                },
            })

        n_pages = len(images)
        instruction = (
            f"Estas son las {n_pages} paginas del mismo analisis clinico, en orden. "
            "Combinalas en un unico resultado consolidado."
            if n_pages > 1 else
            "Extrae todos los valores de este analisis clinico."
        )
        ...
        result["_pages_processed"] = n_pages
        logger.info("Analisis extraido correctamente de %d pagina(s). Fuera de rango: %s",
                    n_pages, result.get("valores_fuera_rango", []))
        return result
